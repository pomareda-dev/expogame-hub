    ...
    <div className="min-h-screen w-full bg-slate-900 relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 px-8 py-8 flex justify-between items-center">
         <div className="animate-slide-up" style={{ animationDelay: '0ms' }}>
            <h1 className="text-3xl font-black tracking-tight text-white drop-shadow-lg">ExpoHub<span className="text-brand-500">.os</span></h1>
            <p className="text-slate-300 text-sm mt-1 font-medium tracking-wide">Interactive Experience Center</p>
         </div>
         <div className="px-4 py-2 bg-white/5 backdrop-blur-xl rounded-full border border-white/10 text-xs font-mono text-slate-200 flex items-center gap-2 shadow-lg animate-slide-up" style={{ animationDelay: '100ms' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-5xl w-full perspective-1000">
          {games.map((game, index) => (
            <button
              ...
              style={{ animation: `slideUpFade 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards`, animationDelay: `${150 + index * 100}ms`, opacity: 0 }}
            >
              {/* Glow effect behind icon */}
              <div className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-20 transition-opacity duration-500 blur-xl ${game.color}`}></div>

              <div className={`relative w-20 h-20 md:w-24 md:h-24 rounded-2xl ${game.color} flex items-center justify-center shadow-lg shadow-black/20 group-hover:shadow-xl group-hover:scale-110 transition-all duration-500 z-10`}>
                <div className="transition-transform duration-500 group-hover:rotate-6">
                   {game.icon}
                </div>
              </div>
              <div className="text-center relative z-10">
                <h3 className="text-lg font-bold text-white mb-1 group-hover:text-brand-200 transition-colors">{game.name}</h3>
                <div className="h-6 overflow-hidden relative">
                    <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold transition-all duration-300 absolute w-full top-0 group-hover:-top-6 opacity-100 group-hover:opacity-0">Tap to Play</p>
                    <p className="text-xs text-brand-300 uppercase tracking-wide font-semibold transition-all duration-300 absolute w-full top-6 group-hover:top-0 opacity-0 group-hover:opacity-100">{game.description}</p>
                </div>
              </div>
            </button>
          ))}
          ...
          <div 
            ...
            style={{ animation: `slideUpFade 0.5s cubic-bezier(0.16, 1, 0.3, 1) forwards`, animationDelay: `${150 + games.length * 100}ms` }}
          >
            ...
            <span className="text-sm text-slate-500 font-medium uppercase tracking-widest">Coming Soon</span>
          </div>
        </div>
      </main>

      {/* Footer */}
      <footer className="relative z-10 p-6 text-center animate-slide-up" style={{ animationDelay: '600ms' }}>
