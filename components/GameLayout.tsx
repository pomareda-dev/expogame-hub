...
export const GameLayout: React.FC<GameLayoutProps> = ({ title, children, highScore }) => {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
            ...
            {highScore !== undefined && (
              <div className="flex items-center gap-2 px-3 py-1 bg-yellow-500/10 border border-yellow-500/20 rounded-full w-fit">
                 ...
                 <span className="text-[10px] sm:text-xs font-bold text-yellow-500 uppercase tracking-wider">Best</span>
                 <span className="font-mono text-sm font-bold text-yellow-400 leading-none">{highScore}</span>
              </div>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
           <span className="hidden sm:block text-xs font-mono text-slate-400 uppercase tracking-widest">ExpoGames OS</span>
