...
export const CatcherGame: React.FC = () => {
  ...
  useEffect(() => {
    const img = new Image();
    img.src = "assets/novatech-white.svg";
    starImg.current = img;
  }, []);
  ...
  const update = (canvas: HTMLCanvasElement) => {
    ...
    for (let i = state.items.length - 1; i >= 0; i--) {
      ...
      if (
        item.y + 40 >= canvas.height - 50 &&
        item.y < canvas.height - 10 &&
        item.x + 70 > state.basketX &&
        item.x < state.basketX + state.basketWidth
      ) {
        if (item.type === "good") {
          setScore((s) => {
            const newScore = s + 10;
  ...
  const draw = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    state.items.forEach((item) => {
      if (item.type === "good" && starImg.current) {
        ctx.drawImage(starImg.current, item.x, item.y - 42, 70, 42);
      } else {
        ctx.font = "42px Arial";
    ...
    if (status === "IDLE") {
      ...
      ctx.fillText(
        "Catch NovaTech and Avoid Bombs 💣",
        canvas.width / 2,
        canvas.height / 2 + 30
      );
  ...
  return (
    <GameLayout title="Star Catcher" highScore={highScore}>
      ...
      <ResultModal
        ...
        message={
          lives > 0
            ? "Great job! You survived the time limit."
